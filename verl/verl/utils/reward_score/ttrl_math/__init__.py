...
def compute_score(model_response, gt_answer, fast=False):
    # [修改点 1] 增加 try-except 包裹，防止 grade 函数内部超时炸断进程
    try:
    ...
    # [修改点 2] 捕获超时错误，返回 0 分而不是报错
    except TimeoutError:
        print(f"[WARN] Grading Timeout for response prefix: {str(model_response)[:50]}...")
        return {
            "score": 0.0,
            "format_score": 0.0,
            "acc": False,
        ...
        print(f"[WARN] Grading Exception: {e}")
        return {
            "score": 0.0,
            "format_score": 0.0,
            "acc": False,
...
def reward_func(
    ...
    try:
        ...
        # [修改点 3] 仅仅打印错误日志，返回 0.0，千万不要 raise！
        print(f"[ERROR] Error in reward_func for task: {str(e)}")
        # traceback.print_exc() # 可选：注释掉这个以减少刷屏
        return 0.0 # 兜底返回 0 分